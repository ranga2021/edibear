...
if ($main_cat_id === '' && $product_category_id !== '' && EdiExplorerContent::columnExists($conn, "books_details", "product_category_id")) {
    $conditions["product_category_id"] = (int) $product_category_id;
    if ((int) $product_subcategory_id > 0 && EdiExplorerContent::columnExists($conn, "books_details", "product_subcategory_id")) {
        $conditions["product_subcategory_id"] = (int) $product_subcategory_id;
    }
}

// Breadcrumb + tag links: section and categories only (language/grade stay in URL for deep links, not shown as crumbs).
...
if($sub_cat_id != ""){
    ...
    $booksPreserveParams = EdiContentTags::preserveListParams($language, $grade, $main_cat_id, $sub_cat_id);
    if ($main_cat_id === '' && $product_category_id !== '' && (int) $product_category_id > 0) {
        $booksPreserveParams["product_category_id"] = (string) (int) $product_category_id;
    }
    if ($main_cat_id === '' && (int) $product_subcategory_id > 0) {
        $booksPreserveParams["product_subcategory_id"] = (string) (int) $product_subcategory_id;
    }
    ...
    $booksPageHeroTitle = "BOOKS & PAPERS";
    if ($ediBcShowSub) {
        $booksPageHeroTitle = $ediBcSubRaw;
    } elseif ($ediBcShowMain) {
        $booksPageHeroTitle = $mainCatTitle;
    }
...
<body>
    ...
    <div class="container mt-5 page-header-content">
            ...
            <div class="edi-page-title-row mt-2">
                        <h1><?php echo htmlspecialchars($booksPageHeroTitle, ENT_QUOTES, 'UTF-8'); ?></h1>
            ...
            <?php
            $tagRowsForCloud = $user->fetchAll(array("tag"), array("books_details"), $conditions, "", $listComboForTagCloud);
            $tagsArr = EdiContentTags::distinctFromRows($tagRowsForCloud);
            ...
            for ($i = 0; $i < $visibleTags; $i++) {
                ...
                $tagHref = "./books.php?" . http_build_query($tagQ, "", "&", PHP_QUERY_RFC3986);
                $safeTag = htmlspecialchars($tag, ENT_QUOTES, "UTF-8");
                $safeHref = htmlspecialchars($tagHref, ENT_QUOTES, "UTF-8");
                echo "<a style='color:#000; border-color:#a7a7a7; border-style:solid; border-width: 1px;' class='tagfont px-3 py-1 mr-1 mb-2' href='" . $safeHref . "'>" . $safeTag . "</a>";
            }
            ...
            if ($totalTags > $visibleTags) {
                ...
                for ($i = $visibleTags; $i < $totalTags; $i++) {
                    ...
                    $safeTag = htmlspecialchars($tag, ENT_QUOTES, "UTF-8");
                    $safeHref = htmlspecialchars($tagHref, ENT_QUOTES, "UTF-8");
                    echo "<a style='color:#000; border-color:#a7a7a7; border-style:solid;' class='tagfont px-3 py-1 mr-1 mb-2 mt-1' href='" . $safeHref . "'>" . $safeTag . "</a>";
                }
                echo "</div>";
                echo "</div>";
