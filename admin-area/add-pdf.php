...
$ediHasPdfOriginalName = EdiExplorerContent::ensureNullableTextColumn($ediConn, "pdf_details", "pdf_original_name");
$ediLanguages = EdiTaxonomy::loadLanguages($ediConn);
$ediGrades = EdiTaxonomy::loadGrades($ediConn, true);
$ediCurLanguageId = 0;
$ediCurGradeId = 0;
...
$ediCurWsCat = 0;
$ediCurWsSub = 0;
$ediPdfWsColsOk = false;
if ($ediHasWsTaxonomy) {
    $ediPdfWsColsOk = EdiWsTaxonomy::ensureWorksheetWsColumns($ediConn, "pdf_details");
}
if ($ediHasPcat) {
    try {
...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    ...
    if ($user->CountRows("pdf_details", array("id"=>$currentpdfID))) {
        ...
        if ($ediHasWsTaxonomy && $ediPdfWsColsOk && EdiExplorerContent::columnExists($ediConn, "pdf_details", "ws_category_id")) {
            $r2 = $ediConn->query(
                "SELECT `ws_category_id`, `ws_subcategory_id` FROM `pdf_details` WHERE `id` = " . (int) $currentpdfID
            );
            if ($r2) {
...
if (isset($_POST['addNewpdfSubmit']) || isset($_POST['updatepdfSubmit'])) {
    ...
    if (isset($_POST['addNewpdfSubmit'])) {
        ...
        if (!empty($_FILES["inputpdfpdfupload"]["name"])) {
            $pdfOriginalName = trim((string) $_FILES["inputpdfpdfupload"]["name"]);
        ...
        if ($ediHasPdfOriginalName) {
            $insertRow["pdf_original_name"] = $pdfOriginalName;
        }
        if ($ediHasWsTaxonomy && $ediPdfWsColsOk) {
            ...
            $insertRow["ws_category_id"] = $wsPair[0];
            $insertRow["ws_subcategory_id"] = $wsPair[1];
        }
        $pdfID = $user->insertTable("pdf_details", $insertRow, true);

        edi_regenerate_public_sitemap($user);
        edi_admin_flash_success('PDF added successfully.');
        $user->redirect('./pdf');
    }
    ...
    if (isset($_POST['updatepdfSubmit'])) {
        ...
        if ($ediHasWsTaxonomy && EdiWsTaxonomy::ensureWorksheetWsColumns($ediConn, "pdf_details")) {
            $wsPair = EdiWsTaxonomy::normalizeWorksheetWsIds(
                $ediConn,
                isset($_POST["edi_ws_category_id"]) ? (int) $_POST["edi_ws_category_id"] : 0,
                isset($_POST["edi_ws_subcategory_id"]) ? (int) $_POST["edi_ws_subcategory_id"] : 0
            );
            $upRow["ws_category_id"] = $wsPair[0];
            $upRow["ws_subcategory_id"] = $wsPair[1];
        }
        ...
        if (!empty($_FILES["inputpdfpdfupload"]["name"])) {
            ...
            if ($ediHasPdfOriginalName) {
                $upPdf["pdf_original_name"] = trim((string) $_FILES["inputpdfpdfupload"]["name"]);
            }
            $user->updateTable("pdf_details", $upPdf, array("id"=>$currentpdfID));
        }

        edi_regenerate_public_sitemap($user);
        edi_admin_flash_success('PDF updated successfully.');
        $user->redirect('./pdf');
    }
}
?>
...
<?php
if ($editMode) {
    echo "<button type='submit' name='updatepdfSubmit' class='btn btn-success'>Update</button>";
    echo "<button type='button' class='btn btn-danger' onclick='deletepdfSubmit()'>Delete</button>";
} else {
