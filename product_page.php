...
<body>
    ...
    <div class="container mt-5 page-header-content" style="margin-top: 0px !important;">
         <div class="col-lg-8 ">
                <i class="fa fa-home pt-1 pr-2 text-primary1"></i><a href="./">Home</a><i class="fa fa-angle-right pt-1 px-2 text-primary1"></i>HONEY CART
                 
                  <!-- Title + Line -->
        <div style="display:flex; align-items:center; gap:15px; margin-bottom:20px;">
            <h2 style="font-weight:700; margin:0;">TREASURES</h2>
        ...
        <form method="GET" action="" class="filter-wrapper row mb-4 align-items-center" style='margin-bottom: 0rem !important;'>
    ...
    <div class="filter-wrapper">
        <select name="category" class="form-control" onchange="this.form.submit()">
            ...
            <?php foreach($categories as $c): ?>
                <option value="<?= $c['id'] ?>" <?= ($catF==$c['id'])?'selected':'' ?>>
                    <?= $c['name'] ?>
                </option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="col-md-2">
        <select name="age" class="form-control" onchange="this.form.submit()">
            ...
            <?php foreach($ageGroups as $a): ?>
                <option value="<?= $a['age_group'] ?>" <?= ($ageF==$a['age_group'])?'selected':'' ?>>
                    <?= $a['age_group'] ?>
                </option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="col-md-2">
        <select name="brand" class="form-control" onchange="this.form.submit()">
            ...
            <?php foreach($brands as $b): ?>
                <option value="<?= $b['brand'] ?>" <?= ($brandF==$b['brand'])?'selected':'' ?>>
                    <?= $b['brand'] ?>
                </option>
            <?php endforeach; ?>
        </select>
    ...
    <!-- OFFERS -->
    <div class="col-md-2">
        <select name="offers" class="form-control" onchange="this.form.submit()">
